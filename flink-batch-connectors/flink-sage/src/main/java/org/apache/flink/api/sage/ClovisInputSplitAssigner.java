...
public class ClovisInputSplitAssigner implements InputSplitAssigner {
	
	/** The logging object used to report information and errors. */
	private static final Logger LOG = LoggerFactory.getLogger(ClovisInputSplitAssigner.class);
	
	/** The list of all splits */
	private final List<InputSplit> splits = new ArrayList<InputSplit>();
	
	public ClovisInputSplitAssigner(InputSplit[] splits) {
		Collections.addAll(this.splits, splits);
	}
	
	public ClovisInputSplitAssigner(Collection<? extends InputSplit> splits) {
